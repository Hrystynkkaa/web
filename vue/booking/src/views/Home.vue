...
export default {
  ...
  created() {
    this.fetchProperties(); // Завантажте квартири при створенні компонента
  },
  methods: {
    async fetchProperties() {
      try {
        const response = await axios.get('/api/apartments'); // Отримайте квартири з бекенду
        this.properties = response.data; // Збережіть їх у стані
      } catch (error) {
        console.error('Не вдалося отримати квартири:', error);
      }
    },
    performSearch(criteria) {
      // Логіка для обробки критеріїв пошуку
      console.log('Критерії пошуку:', criteria);
      // Тут можна фільтрувати список квартир на основі критеріїв
    },
  },
};
</script>
...
/* Додаткові стилі для адаптації карток */
.property-card {
...
.property-card .property-details {
  padding: 15px; /* Внутрішнє відступи */
}
